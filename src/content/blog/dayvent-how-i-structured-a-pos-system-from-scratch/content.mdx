...
Semanas después, la empresa se queja: les llegó una factura de $3.000 USD. Y ahí te preguntas: _"¿Qué habré hecho mal?"_.
...
```mermaid
flowchart LR
    C["Clientes en hora pico"] --> GW["API Gateway"]
    GW --> L1["Lambda #1"]
    GW --> L2["Lambda #2"]
    GW --> L3["Lambda #N"]
    L1 --> P[("Prisma Client")]
    L2 --> P
    L3 --> P
    P --> DB[("PostgreSQL RDS<br/>límite de conexiones")]
    DB -. excede el pool .-> T["Timeouts"]
```
...
sequenceDiagram
    ...
    U->>CR: POST /venta
    CR-->>U: 200 OK (respuesta inmediata)
    CR->>Q: encola tarea asíncrona
    Note over Q: controla ritmo,<br/>concurrencia y reintentos
    Q->>CR: POST /tasks/procesar (nueva petición)
    CR->>CR: procesa con memoria fresca
```
...
```mermaid
flowchart TD
    POS["POS / Frontend React"] -->|ventas en tiempo real| CR["Cloud Run · Go<br/>autoescalado, escala a cero"]
    CR -->|OLTP: lecturas y escrituras| PG[("PostgreSQL<br/>tablas por tenant_id")]
    CR -->|trabajo asíncrono| Q["Cloud Tasks"]
    Q -->|reintenta vía HTTP| CR
    PG -->|replicación analítica| CH[("ClickHouse<br/>columnar · OLAP")]
    DASH["Dashboards y reportes"] -->|consultas pesadas| CH
```
...
- [Prisma: AWS RDS Proxy fija las conexiones (sin beneficio de pooling con Prisma Client)](https://www.prisma.io/docs/orm/prisma-client/setup-and-configuration/databases-connections/configure-pg-bouncer#aws-rds-proxy)
- [AWS Lambda: reutilización del entorno de ejecución](https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtime-environment.html)
